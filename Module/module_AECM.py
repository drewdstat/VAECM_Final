...
class AECM(tf.keras.Model):
    def __init__(self, architecture, n_clusters, true_labels=None, beta=1., lbd=1.,tracker=''):
        #added ,tracker='' to match module_CM.py
    ...
    def loss_cm(self, z, alpha, training=False):
        ...
        loss_op1 = tf.reduce_sum( tf.math.squared_difference(x, tx) ) #E1 Reconstruction loss CM
        ...
        loss_op4 = tf.reduce_sum( tf.math.log( tf.reduce_mean(g,0) +1e-10 ) * (1-alpha)) #E4 
    ...
    def loss(self, x, alpha, training=False):
        ...
        loss_op1 = tf.reduce_sum( tf.math.squared_difference(c, tc) ) #E0/1 Reconstruction loss CM 
        loss_op2 = tf.reduce_sum( tf.reduce_sum( g*(1-g),0 ) ) #Sparsity
        
        uu = tf.linalg.matmul( u,u, transpose_b=True )
        loss_op2b = tf.reduce_sum( tf.abs(uu-tf.eye(self.n_clusters)) ) * self.lbd #Orthonormality
        
        lmg = tf.math.log( tf.reduce_mean(g,0) +1e-10 )
        
        slmg = tf.sort(lmg)

        loss_op3 = tf.reduce_sum( slmg * (1-alpha) ) #E3/4 Dirichlet prior
#        for k in range(self.n_clusters):
#            loss_op3 += gm[ogm[k]] * (1-alpha[k])
        
        loss_op4 = tf.reduce_sum( tf.math.squared_difference(x, tx) ) * self.beta #E0/1 Reconstruction loss AE

        return tf.stack( (loss_op1 , loss_op2 , loss_op2b , loss_op3 , loss_op4) )
    
    # @tf.function
    # def Clustering_Module_Loss(self, x, alpha, training=False):
    #     n,d = x.shape
    #     nd = (n*d)
    #     c  = self.encode(x, training)
    #     g  = self.gamma( c, training)
